...
{
    if (pin > 3 || pin < 1)
        throw nts::Error("Invalid pin on Or component.");
    if (pin == 1 || pin == 2) {
        if (!pins[pin].targetComp)
            throw nts::Error("Pin " + std::to_string(pin) + " on Or component is not link.");
        return pins[pin].targetComp->compute(pins[pin].targetPin);
    }
    if (!pins[1].targetComp)
            throw nts::Error("Pin 1 on Or component is not link.");
    if (!pins[2].targetComp)
            throw nts::Error("Pin 2 on Or component is not link.");
